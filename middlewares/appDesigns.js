...
const filterAndSort = (req, res, next) => {
  ...
  req.findFilter = {};
  if (author) req.findFilter.creator = new RegExp(`${author}`, 'gi');
  if (title) req.findFilter.title = new RegExp(`${title}`, 'gi');
  if (tags) {
    const tagAttributes = tags
      .split(',')
      .map((tag) => new RegExp(`${tag}`, 'gi'));
  ...
  if ('author' in req.sort) {
    req.sort.creator = req.sort.author;
...
const list = (req, res, next) => {
  ...
  req.fields === undefined
    ? (req.fields = fields)
    : (req.fields = Object.assign(req.fields, fields));
  next();
};

const setUserFilter = (req, res, next) => {
  const creator = new RegExp(`${req.user.username}`, 'gi');
  req.findFilter.creator = creator;
  next();
};
